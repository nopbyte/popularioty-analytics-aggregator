...
public class SearchAndStorageInstanceBuilder 
{
	private static Logger LOG = LoggerFactory.getLogger(SearchAndStorageInstanceBuilder.class);	
	

	private static SearchAndStorageInstanceBuilder instance;
	...
	protected SearchAndStorageInstanceBuilder()
	{
		searchProviders = new HashMap<>();
		storageProviders = new HashMap<>();
	}
	
	public synchronized StorageProvider getStorageProvider(String configurationFile) {
		return storageProviders.get(configurationFile);
	}
	/**
	 ...
	 */
	public static SearchAndStorageInstanceBuilder getInstance( ) {
		   
	      if(instance == null) {
	    	  synchronized(SearchAndStorageInstanceBuilder.class)
			   {  
	    		  instance = new SearchAndStorageInstanceBuilder();
			   }
	      }
	      return instance;
	}
